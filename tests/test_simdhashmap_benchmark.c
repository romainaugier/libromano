...
int main(int argc, char** argv)
{
    ...
    if(TEST_NAME(test_name, "insert_random_shuffle_range"))
    {
        ...
        {
            simdhashmap_set(hashmap, (const void*)vector_at(keys_insert, i), sizeof(uint64_t), &one, sizeof(uint64_t));
        }
    ...
    else if(TEST_NAME(test_name, "read_random_shuffle_range"))
    {
        ...
        for(i = 0; i < num_keys; i++)
        {
            simdhashmap_get(hashmap, (const void*)vector_at(keys_insert, i), sizeof(uint64_t), NULL);
        }
    ...
    else if(TEST_NAME(test_name, "insert_random_full_reserve"))
    {
        ...
        hashmap = simdhashmap_new(num_keys);
        simdhashmap_set_hash_func(hashmap, hash_identity);
    ...
    else if(TEST_NAME(test_name, "read_miss_random_full"))
    {
        ...
        for(i = 0; i < num_keys; i++)
        {
            simdhashmap_get(hashmap, (const void*)vector_at(keys_read, i), sizeof(uint64_t), NULL);
        }
    ...
    else if(TEST_NAME(test_name, "read_random_full_after_delete"))
    {
        ...
        for(i = 0; i < num_keys; i++)
        {
            simdhashmap_remove(hashmap, (const void*)vector_at(keys_insert, i), sizeof(uint64_t));
        }
