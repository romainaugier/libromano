...
#define PROFILE_MICROSECONDS(func) { uint64_t s = __rdtsc();                                                                                     \
                                     do { func; } while (0);                                                                                     \
                                     printf("%s at %s:%d -> %3f µs\n", #func, __FILE__, __LINE__, (float)((uint64_t)__rdtsc() - s) / 1000.0f); } \

#define SCOPED_PROFILE_START_MICROSECONDS(name) const char* ___scp_##name = #name; uint64_t ___scp_##name##_start = __rdtsc(); 
#define SCOPED_PROFILE_END_MICROSECONDS(name) printf("Scoped profile \"%s\" -> %3f µs\n", ___scp_##name, (float)((uint64_t)__rdtsc() - ___scp_##name##_start) / 1000.0f); 

/* Profiling measured in milliseconds */
#define PROFILE_MILLISECONDS(func) { uint64_t s = __rdtsc();                                                                                        \
                                     do { func; } while (0);                                                                                        \
                                     printf("%s at %s:%d -> %3f ms\n", #func, __FILE__, __LINE__, (float)((uint64_t)__rdtsc() - s) / 1000000.0f); } \

#define SCOPED_PROFILE_START_MILLISECONDS(name) const char* ___scp_##name = #name; uint64_t ___scp_##name##_start = __rdtsc(); 
#define SCOPED_PROFILE_END_MILLISECONDS(name) printf("Scoped profile \"%s\" -> %3f ms\n", ___scp_##name, (float)((uint64_t)__rdtsc() - ___scp_##name##_start) / 1000000.0f); 

/* Profiling measured in seconds */
#define PROFILE_SECONDS(func) { uint64_t s = __rdtsc();                                                                                          \
                                do { func; } while (0);                                                                                          \
                                printf("%s at %s:%d -> %3f s\n", #func, __FILE__, __LINE__, (float)((uint64_t)__rdtsc() - s) / 1000000000.0f); } \

#define SCOPED_PROFILE_START_SECONDS(name) const char* ___scp_##name = #name; uint64_t ___scp_##name##_start = __rdtsc(); 
#define SCOPED_PROFILE_END_SECONDS(name) printf("Scoped profile \"%s\" -> %3f s\n", ___scp_##name, (float)((uint64_t)__rdtsc() - ___scp_##name##_start) / 1000000000.0f); 
